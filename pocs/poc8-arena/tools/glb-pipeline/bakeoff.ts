...
const RODIN_KEY = process.env.RODIN_KEY ?? '';
...
async function generateTripo(partId: string, prompt: string): Promise<GenerationResult> {
  ...
  try {
    ...
    for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
      ...
      if (status === 'success') {
        const glbUrl = pollData.data.output?.model;
        if (!glbUrl) {
...
async function generateMeshy(partId: string, prompt: string): Promise<GenerationResult> {
  ...
  try {
    ...
    console.log(`  [meshy] ${partId}: task ${taskId} created, polling...`);
    ...
    for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
      ...
      if (status === 'SUCCEEDED') {
        ...
        if (!glbUrl) {
          return { api, partId, glbPath, fileSizeKB: 0, durationMs: Date.now() - start, error: 'No GLB URL in response' };
        }
        const glbRes = await fetch(glbUrl);
        const buffer = Buffer.from(await glbRes.arrayBuffer());
        writeFileSync(glbPath, buffer);
      ...
      if (status === 'FAILED') {
        return { api, partId, glbPath, fileSizeKB: 0, durationMs: Date.now() - start, error: `Task failed: ${pollData.message ?? 'unknown'}` };
      }
    }
    return { api, partId, glbPath, fileSizeKB: 0, durationMs: Date.now() - start, error: 'Timeout waiting for completion' };
  } catch (err: any) {
...
async function generateRodin(partId: string, prompt: string): Promise<GenerationResult> {
  ...
  try {
    ...
    const createRes = await fetch('https://api.hyper3d.ai/api/v2/rodin', {
      method: 'POST',
      headers: {
    ...
    for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
      await sleep(POLL_INTERVAL_MS);
      const pollRes = await fetch(`https://api.hyper3d.ai/api/v2/rodin/status`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${RODIN_KEY}`,
      ...
      if (status === 'Done' || status === 'Completed') {
        // Download the GLB
        const downloadRes = await fetch(`https://api.hyper3d.ai/api/v2/rodin/download`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${RODIN_KEY}`,
...
for (const r of results) {
  const status = r.error ? `ERROR: ${r.error.slice(0, 50)}` : 'OK';
  console.log(
