/**
 * POC 3.5: Multi-User Creatures -- Reducers & Lifecycle
 *
...
/** Generate a random point within the arena circle */
function randomArenaPoint(random: () => number): { x: number; y: number } {
  const angle = random() * Math.PI * 2;
  const r = Math.sqrt(random()) * ARENA_RADIUS;
  return {
    x: WORLD_CENTER + Math.cos(angle) * r,
    y: WORLD_CENTER + Math.sin(angle) * r,
  };
}

/** Clamp a point to stay within the arena circle */
function clampToArena(x: number, y: number): { x: number; y: number } {
  ...
  return { x: WORLD_CENTER + dx * scale, y: WORLD_CENTER + dy * scale };
}
...
/**
 * spawn_creature -- spawn a creature at (x, y) with the given leg count and hue.
 */
export const spawn_creature = spacetimedb.reducer(
  { x: t.f32(), y: t.f32(), legCount: t.u16(), hue: t.f32(), speed: t.f32() },
  (ctx, { x, y, legCount, hue, speed }) => {
    const player = ctx.db.player.identity.find(ctx.sender);
    if (!player) {
    ...
    const target = randomArenaPoint(ctx.random);
    ...
    console.info(
      `[spawn] Player "${player.name}" spawned creature ${row.id} (${legCount} legs, hue ${hue.toFixed(2)}) at (${clamped.x.toFixed(1)}, ${clamped.y.toFixed(1)}).`
    );
  }
);
...
export const move_creature = spacetimedb.reducer(
  ...
  (ctx, { creatureId, targetX, targetY }) => {
    ...
    const clamped = clampToArena(targetX, targetY);
  ...
  (ctx, { arg: _scheduleRow }) => {
    ...
    for (const creature of creatures) {
      ...
      if (dist > ARRIVAL_THRESHOLD) {
        ...
        // Arrived — pick a new random wander target within the arena
        const wander = randomArenaPoint(ctx.random);
        ctx.db.creature.id.update({
          ...creature,
          targetX: wander.x,
          targetY: wander.y,
