...
/** Cardinal directions for spatial port placement */
export type Direction = 'top' | 'bottom' | 'left' | 'right' | 'front' | 'back';

/** Mirror map for bilateral symmetry */
...
/** A spatial port slot on a part definition */
export interface PortSlot {
  /** What type of child can attach here */
  type: PortType;
  /** 3D offset from part center [x, y, z] */
  position: [number, number, number];
  /** Cardinal direction (for orientation and symmetry) */
  direction: Direction;
}
...
export interface PartDefinition {
  ...
  /** Spatial port layout for editor visualization and placement */
  portLayout: PortSlot[];
...
export interface PartInstance {
  ...
  instanceId: string;
  /** Which port slot index on the parent this part occupies (-1 for root) */
  portIndex: number;
